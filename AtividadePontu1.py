...
precos_exames = [35.00, 150.00, 300.00, 175.00, 950.00, 2500.00, 50.00]

# Vetores para armazenar temporariamente as escolhas do paciente
exames_escolhidos_cod = []
exames_escolhidos_nomes = []
subtotal = 0.0

# 2. LAÇO DE REPETIÇÃO: SELEÇÃO DE EXAMES.
while True:
    # Limpa a tela no código 
    os.system('cls' if os.name == 'nt' else 'clear')
    
    # Exibe o menu iterando sobre os vetores
    print("=" * 50)
    print(f"{'VIDA PLENA | AGENDAMENTOS':^50}")
    print("=" * 50)
    print(f"{'CODs'} / {'EXAMES'} / {'PREÇOS'}")
    print("-" * 50)
    
    for i in range(len(codigos_exames)):
        print(f"{codigos_exames[i]:<4} | {nomes_exames[i]:<25} | R$ {precos_exames[i]:>12.2f}")
    ...
    if entrada_opcao in ["0", "1", "2", "3", "4", "5", "6", "7"]:
        ...
        print("\n[Não identificado] Código inválido. Por favor, digite um número presente na lista.")
        time.sleep(0.5)
        continue
    ...
    exames_escolhidos_cod.append(codigos_exames[indice])
    exames_escolhidos_nomes.append(nomes_exames[indice])
    subtotal += precos_exames[indice]
    
    print(f"\n'{nomes_exames[indice]}' adicionado ao agendamento.")
    time.sleep(1)
    ...
    if continuar == "N":
        break  # Sai do laço principal e vai para pagamento do subtotal que foi acumulado

# 3. CONDICIONAIS E PAGAMENTO.
...
if subtotal == 0:
    ...
    while True:
        ...
        if entrada_pagamento in ["1", "2", "3"]:
            ...
            print("\n[Não identificado] Digite apenas 1, 2 ou 3.")
            time.sleep(2)
            continue
    ...
    for i in range(len(exames_escolhidos_cod)):
        print(f" - CODs [{exames_escolhidos_cod[i]:02d}] {exames_escolhidos_nomes[i]}")
    
    print("-" * 50)
    print(f"Subtotal:              R$ {subtotal:10.2f}")
    print(f"Forma de Pagamento:    {forma_pagamento}")
    
    # Valida o que for exibido no recibo
    if desconto > 0:
    ...
    print("=" * 55)
    print("\nObrigado por agendar seus exames no Hospital Vida Plena.")
